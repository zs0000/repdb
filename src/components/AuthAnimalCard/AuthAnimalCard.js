...
import s from "./AuthAnimalCard.module.css"
import Image from "next/image"
import { useEffect, useRef, useState } from "react"
import AuthAnimalCardContextMenu from "../AuthAnimalCardContextMenu/AuthAnimalCardContextMenu"


export default function AuthAnimalCard({animal, handleSelectAnimal, selectedAnimals, actionState, modalIsOpen, setModalIsOpen, action, setAction, actionItems, setActionItems,}) {
  const router=useRouter()
    const [isOpen, setIsOpen] = useState(false)
    const animalCardMenuRef = useRef()

    const handleClickOutsideOfMenu = (e) => {
        if(animalCardMenuRef.current && !animalCardMenuRef.current.contains(e.target)){
            setIsOpen(false)
        }
    }
 ...
 className={s.photo}  />
        ...
        <div className={s.infocontainer}>
            ...
            <AuthAnimalCardContextMenu isOpen={isOpen} setIsOpen={setIsOpen} animalCardMenuRef={animalCardMenuRef} animal_id={animal.animal_id} modalIsOpen={modalIsOpen} setModalIsOpen={setModalIsOpen}
            actionItems={actionItems} setActionItems={setActionItems} action={action} setAction={setAction} actionState={actionState}
            />
